...
class PhaseDetector:
    ...
    def __init__(self, interval_len=10000, stable_min=4, threshold=.5, bits_log2 = 10, drop_bits = 3, binary_filename=None):
        """
        ...
        binary_filename : string
            The filename of the binary that is to be used to determine what functions instructions came from. Disabled if None. Currently only supports ELF/DWARF so please do not attempt to use this on MacOS. (default None)
        """
        ...
        # Open the binary and parse the ELF
        self.dwarfmap = None

        if binary_filename != None:
            self.dwarfmap = DWARFMap.DWARFMap(binary_filename)
    ...
    def reset(self):
        ...
        self.dwarf_count = {}
        self.ip_cache    = {}
        self.hist        = numpy.zeros(self.interval_len, dtype=numpy.uint64)
    ...
    def access(self, ip: numpy.uint64) -> None:
        ...
        # Store the addresses for later classification
        self.hist[numpy.uint64(self.naccesses % self.interval_len)] = ip
        ...
        if self.naccesses % self.interval_len == 0 and self.naccesses != 0:
            ...
            # Add addr info to each phase
            # Don't save info if phase is not identified or we don't have DWARF info
            if self.phase != -1 and self.dwarfmap != None:

                # If we've saved info from this phase before, we can just add to it by passing it in as a param
                if self.phase not in self.dwarf_count:
                    self.dwarf_count[self.phase] = self.dwarfmap.classify(self.hist)
                #TODO: uncomment if necessesary
                #else:
                #    self.dwarf_count[self.phase] = self.dwarfmap.classify(self.hist, self.dwarf_count[self.phase])
